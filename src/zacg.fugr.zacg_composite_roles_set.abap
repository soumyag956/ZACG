FUNCTION zacg_composite_roles_set.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
*"     VALUE(IT_COMP) TYPE  ZACG_T_COMP_SET
...
*"----------------------------------------------------------------------
  ...
  DATA:
    ...
    lr_low_tcode                   TYPE RANGE OF agval,
  ...
  SELECT agr_name, object, auth, field, low, high
    ...
    WHERE agr_name  EQ @it_comp-child_agr
  ...
  IF sy-subrc IS INITIAL.
    ...
    IF lr_tcode IS NOT INITIAL.
      ...
      LOOP AT it_comp[] INTO DATA(lwa_comp).

        CALL FUNCTION 'ZACG_COMPOSITE_ROLE'
          EXPORTING
            iv_role           = lwa_comp-child_agr
            it_tcode          = lr_tcode
          IMPORTING
      ...
      IF lr_func IS NOT INITIAL AND lr_tcode IS NOT INITIAL.
        ...
        IF lr_func IS NOT INITIAL.
          ...
          READ TABLE it_comp[] INTO lwa_comp INDEX 1.
          CLEAR lit_potential_risk2.
            ...
            IF sy-subrc IS INITIAL.
              ...
              LOOP AT lit_potential_risk INTO DATA(lwa_potential_risk1) FROM sy-tabix.
                ...
                IF sy-subrc IS INITIAL.
                  ...
                  <lfs_risk_summary>-composite  = lwa_comp-agr_name.
          ...
          IF iv_detail = abap_true.
            ...
            SORT lit_potential_risk BY pfcg_agr_name risk.
            ...
            LOOP AT et_risk_summary INTO DATA(lwa_risk_summary).
              ...
              READ TABLE lit_potential_risk TRANSPORTING NO FIELDS WITH KEY pfcg_agr_name = lwa_risk_summary-agr_name
                                                                            risk = lwa_risk_summary-risk BINARY SEARCH.
              IF sy-subrc IS INITIAL.
                ...
                LOOP AT lit_potential_risk INTO DATA(lwa_potential_risk) FROM lv_tabix1.
                  ...
                  ELSE.
                    ...
                    <lfs_risk_detail>-composite = lwa_risk_summary-composite.
  ...
  SELECT *
    FROM @et_risk_detail AS blank_tcode
*    WHERE tcode IS INITIAL
  INTO TABLE @DATA(li_risk_detail).
  IF sy-subrc IS INITIAL.

    SORT li_risk_detail BY user composite agr_name risk func.
    DELETE ADJACENT DUPLICATES FROM li_risk_detail COMPARING user composite agr_name risk func.

    LOOP AT li_risk_detail ASSIGNING <lfs_risk_detail>.
      READ TABLE et_risk_summary TRANSPORTING NO FIELDS
      WITH KEY risk = <lfs_risk_detail>-risk type = abap_true.
      IF sy-subrc IS INITIAL.
        <lfs_risk_detail>-risk = space.
      ELSE.

        SELECT *
          FROM @lit_potential_risk AS missing
          WHERE pfcg_agr_name = @<lfs_risk_detail>-agr_name
            AND risk          = @<lfs_risk_detail>-risk
            AND lib_func      = @<lfs_risk_detail>-func
            AND lib_object    = @<lfs_risk_detail>-object
            AND lib_field     = @<lfs_risk_detail>-field
        INTO TABLE @lit_potential_risk1.

        LOOP AT lit_potential_risk1 INTO lwa_potential_risk1.
          APPEND INITIAL LINE TO lit_risk_detail ASSIGNING FIELD-SYMBOL(<lfs_risk_detail1>).
          <lfs_risk_detail1> = <lfs_risk_detail>.
          <lfs_risk_detail1>-tcode = lwa_potential_risk1-lib_tcode.
        ENDLOOP.
      ENDIF.
    ENDLOOP.
    li_risk_detail = lit_risk_detail.
    CLEAR lit_risk_detail.

    LOOP AT li_risk_detail INTO DATA(lwa_risk_detail).

      SELECT *
        FROM @et_risk_detail AS all_detail
        WHERE user = @lwa_risk_detail-user
          AND composite = @lwa_risk_detail-composite
          AND agr_name NE @lwa_risk_detail-agr_name
          AND risk = @lwa_risk_detail-risk
      INTO TABLE @DATA(li_missing_risk_detail).
      IF sy-subrc IS INITIAL.
        LOOP AT li_missing_risk_detail INTO DATA(ls_risk_detail).

          CLEAR: lit_agr1251_tcode, lr_low.
          SELECT *
            FROM @lit_agr1251_detail AS tcode
            WHERE agr_name EQ @ls_risk_detail-agr_name
              AND object = 'S_TCODE'
            INTO TABLE @lit_agr1251_tcode.
          LOOP AT lit_agr1251_tcode INTO lwa_agr1251_tcode.
            IF lwa_agr1251_tcode-high IS NOT INITIAL.
              lr_low  = VALUE #( ( sign = 'I' option = 'BT' low = lwa_agr1251_tcode-low high = lwa_agr1251_tcode-high ) ).
            ELSEIF lwa_agr1251_tcode-low CA '*'.
              lr_low = VALUE #( ( sign = 'I' option = 'CP' low = lwa_agr1251_tcode-low ) ).
            ELSE.
              lr_low = VALUE #( ( sign = 'I' option = 'EQ' low = lwa_agr1251_tcode-low ) ).
            ENDIF.
            IF lwa_risk_detail-tcode IN lr_low.
              APPEND INITIAL LINE TO lit_risk_detail ASSIGNING <lfs_risk_detail>.
              <lfs_risk_detail> = ls_risk_detail.
              <lfs_risk_detail>-func  = lwa_risk_detail-func.
              <lfs_risk_detail>-funcd = lwa_risk_detail-funcd.
              <lfs_risk_detail>-tcode = lwa_risk_detail-tcode.
              CLEAR: <lfs_risk_detail>-object,
                     <lfs_risk_detail>-auth,
                     <lfs_risk_detail>-field,
                     <lfs_risk_detail>-low,
                     <lfs_risk_detail>-high.
              EXIT.
            ENDIF.
          ENDLOOP.
    ...
    LOOP AT lit_risk_detail INTO lwa_risk_detail.
      READ TABLE et_risk_detail[] TRANSPORTING NO FIELDS
      WITH KEY user = lwa_risk_detail-user
               composite = lwa_risk_detail-composite
               agr_name = lwa_risk_detail-agr_name
               risk = lwa_risk_detail-risk
               func = lwa_risk_detail-func
               tcode = lwa_risk_detail-tcode.
      IF sy-subrc IS NOT INITIAL.
        APPEND lwa_risk_detail TO et_risk_detail[].
      ENDIF.
    ENDLOOP.

  ENDIF.

  SORT et_risk_detail   BY composite agr_name risk func tcode object field low high.
  DELETE ADJACENT DUPLICATES FROM et_risk_detail COMPARING composite agr_name risk func tcode object field low high.
  CLEAR: lit_risk_detail, li_risk_detail.
  SELECT *
    FROM @et_risk_detail AS blank_tcode
    WHERE tcode IS INITIAL
  INTO TABLE @li_risk_detail.
  IF sy-subrc IS INITIAL.
    SELECT *
      FROM @et_risk_detail AS blank_tcode
      WHERE tcode IS NOT INITIAL
    INTO TABLE @lit_risk_detail.
    IF sy-subrc IS INITIAL.
      LOOP AT li_risk_detail ASSIGNING <lfs_risk_detail>.
        READ TABLE lit_risk_detail TRANSPORTING NO FIELDS
        WITH KEY user      = <lfs_risk_detail>-user
                 composite = <lfs_risk_detail>-composite
                 agr_name  = <lfs_risk_detail>-agr_name
                 risk      = <lfs_risk_detail>-risk
                 func      = <lfs_risk_detail>-func
                 object    = <lfs_risk_detail>-object
                 field     = <lfs_risk_detail>-field
                 low       = <lfs_risk_detail>-low.
        IF sy-subrc IS INITIAL.
          <lfs_risk_detail>-risk = space.
        ENDIF.
      ENDLOOP.
      DELETE li_risk_detail WHERE risk = space.
      APPEND LINES OF li_risk_detail TO lit_risk_detail.
      et_risk_detail = lit_risk_detail.
    ENDIF.
  ENDIF.


  SORT et_risk_summary  BY composite agr_name risk.
  DELETE ADJACENT DUPLICATES FROM et_risk_summary COMPARING composite agr_name risk.
